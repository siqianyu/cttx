<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ActivityTaskList.aspx.cs" Inherits="AppModules_Member_MemberList" %>
            ...
            <tr>
                <td style=" width:80%">
                    <div class="divSearch" style='width: 600px'>
                        <span class="spanSearch">任务名称：</span>
                        <input style="width: 100px;" id="task_name" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'TaskConfigList.aspx?flag=list',
        datatype: "json",
        colNames: ['任务ID', '任务编码', '任务序号', '任务名称', '备注', '是否启用', '操作'],
        colModel: [
            { name: 'task_id', index: 'task_id', width: 60 },
            { name: 'task_code', index: 'task_code', width: 60, align: 'center' },
            { name: 'task_sn', index: 'task_sn', width: 60, align: 'center', },
   		    { name: 'task_name', index: 'task_name', width: 60, align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "task_id");
            }
        },
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    var id = rowData.task_id;
                    var if_use = rowData.if_use == 1 ? "是" : "否";
                    var writeData = {
                        if_use: if_use,
                        cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='环节配置' onclick=\"button_actions('editnode','" + id + "')\">"
                    }
    ...
    function grid_search() {
        ...
        var _searchfilter = "task_name$$" + task_name + "_$$_if_use$$" + if_use;  //+ "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "TaskConfigList.aspx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'task_id') + "|"; //获取所有选中的id值
                }
                var str = $.ajax({ url: "TaskConfigList.aspx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
    ...
    function edit_method(id) {
        ...
        $.layer({
            ...
            title: [title, true],
            maxmin: true,
            iframe: { src: 'TaskConfigDetail.aspx?id=' + id },
    ...
    function node_method(id) {
        $.layer({
            ...
            title: ['环节配置', true],
            maxmin: true,
            iframe: { src: 'TaskNodeConfigList.aspx?task_id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
